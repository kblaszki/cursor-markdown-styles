...
- **Released:** [`themes/mpe/released/cpp-modern/`](themes/mpe/released/cpp-modern) — recommended dark C++ Modern (`style.less` + `config.js`)
...
5. Replace [`config.js`](themes/mpe/released/cpp-modern/config.js) via **Open Config Script (Global)** — paste the package `config.js` as-is (see MPE README).
