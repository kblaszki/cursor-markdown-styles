# lumina-v5-aurora (experimental)

Paste [`style.less`](style.less) into Crossnote global CSS.

Mermaid / Crossnote: paste [`config.js`](config.js) over `%USERPROFILE%\.crossnote\config.js` (`darkMode: true`, Aurora palette). Pair with this package’s `style.less`.
